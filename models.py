...
@register_model
def deit_small_patch16_224(img_size, depth, localvit, localvit_act, postnorm, 
    layerscale, ls_init, pretrained=False, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=16, embed_dim=384, depth=depth, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), post_norm = postnorm, layerscale=layerscale, ls_init=ls_init, 
        localvit = localvit, localvit_act=localvit_act, **kwargs)   
    model.default_cfg = _cfg(mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD)
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
