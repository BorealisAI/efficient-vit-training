...
#####################################################################################
# Code is based on LocalViT from https://github.com/ofsoundof/LocalViT/tree/main
#################################################################################### 
import torch
...
class LocalityFeedForward(nn.Module):
    def __init__(self, in_dim, out_dim, stride, expand_ratio=4., act='hs+se', reduction=4,
                 wo_dp_conv=False, dp_first=False, layerscale=False, gamma_2=None, layernorm=None, droppath=None):
        """
        ...
        self.hidden_dim = int(in_dim * expand_ratio)
        kernel_size = 3
        ...
        ## change from original code
        if act.find('hs') >= 0:
            act_layer_dp = h_swish()
        else:
            act_layer_dp = nn.ReLU6(inplace=True)
        ## 
        ...
        if not wo_dp_conv:
            if self.isln:
                dp = [
                    nn.Conv2d(self.hidden_dim, self.hidden_dim, kernel_size, stride, kernel_size // 2, groups=self.hidden_dim, bias=False),
                    act_layer_dp
                ]
            else:
                dp = [
        ...
        if act.find('+') >= 0:
            attn = act.split('+')[1]
            if attn == 'se':
                layers.append(SELayer(self.hidden_dim, reduction=reduction))
            elif attn.find('eca') >= 0:
                layers.append(ECALayer(self.hidden_dim, sigmoid=attn == 'eca'))
            else:
                raise NotImplementedError('Activation type {} is not implemented'.format(act))
       

        self.conv = nn.Sequential(*layers)
        self.is_layerscale=layerscale
        if self.is_layerscale:
            self.gamma_2 = gamma_2

        self.fc1 = nn.Linear(in_dim, self.hidden_dim, bias = False)
        ...
        self.fc2 = nn.Linear(self.hidden_dim, out_dim, bias=False)
    ...
    def forward(self, x): 
        batch_size, embed_dim, patch_size, patch_size=x.shape
        if self.isln:
            #('ln')
            x2 = self.layernorm(x.flatten(2).transpose(1, 2))
            x2 = self.fc1(x2)
        else:
            #('bn')
            x2 = self.fc1(x.flatten(2).transpose(1, 2))
            x2 = self.bn1(x2.transpose(1, 2)).transpose(1, 2)
        x2 = self.act1(x2).transpose(1, 2).view(batch_size, self.hidden_dim, patch_size, patch_size)
        x2 = self.conv(x2)
        x2 = self.fc2(x2.flatten(2).transpose(1, 2))
        if self.isln:
            #('ln')
            x2 = x2.transpose(1,2).view(batch_size, embed_dim, patch_size, patch_size)
        else:
            #('bn')
            x2 = self.bn2(x2.transpose(1, 2)).view(batch_size, embed_dim, patch_size, patch_size)
        
        if self.is_layerscale:
            if self.isdrop:
                #('ls+drop')
                x = x + self.droppath((self.gamma_2 * x2.permute(0, 2, 3, 1))).permute(0, 3, 1, 2)
            else:
                #('ls')
                x = x + (self.gamma_2 * x2.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        else:
            if self.isdrop:
                #('drop')
                x = x + self.droppath(x2)
            else:
                #('lclin')
                x = x + x2
        return x
